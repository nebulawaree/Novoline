...
local PlayerUtility = loadstring(readfile("Novoline/Librarys/Utility.lua"))()
local WhitelistModule = loadstring(readfile("Novoline/Librarys/Whitelist.lua"))()
local weaponMeta = HttpService:JSONDecode(game:HttpGet("https://raw.githubusercontent.com/nebulawaree/test/main/sword.json"))
print("wave")
...
local Window = GuiLibrary:CreateWindow({
    Name = "Novoline",
    LoadingTitle = "Novoline Interface",
    LoadingSubtitle = "lol",
    ConfigurationSaving = {
       Enabled = true,
       FolderName = "Novoline/configs",
       FileName = tostring(shared.NovolinePlaceId) .. ".lua"
    },
    Discord = {
       Enabled = false,
       Invite = "",
...
runcode(function()
    ...
    local function AddCape(character)
        ...
        if not character:FindFirstChild("Cape") then
            CreateCape(character, getcustomasset("Novoline/assets/cape.png"))
        end
    end
...
runcode(function()
    ...
    if not getgenv().ChatSpammer then
        getgenv().ChatSpammer = "Novoline on top"
    end
                ...
                RunLoops:BindToHeartbeat("ChatSpammer", function()
                    if ReplicatedStorage:FindFirstChild('DefaultChatSystemChatEvents') then
                        if tick() - lastSentTime >= ChatSpammerDelay.Value then
                            local message = getgenv().ChatSpammer or "Novoline on top"
...
local commands = {
    ...
    [";server default"] = function()
        ...
        task.wait(1.5)
        Window:Prompt({
            Title = 'Novoline Discord Invitation',
            SubTitle = 'Join the Novoline Discord Server',
            Content = 'You have been invited to the Novoline Discord server. Do you wish to join?',
            Actions = {
                Accept = {
                    Name = 'Accept',
                    Callback = function()
    ...
    [";reveal default"] = function(player)
        local message = "I am using Novoline"
...
local function handlePlayer(player, PlayerAdded)
    ...
    if PlayerAdded and whitelist.players[hashedCombined] and not amIWhitelisted then
        if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
            local newchannel = cloneref(game:GetService('RobloxReplicatedStorage')).ExperienceChat.WhisperChat:InvokeServer(player.UserId)
            if newchannel and player ~= lplr then
                newchannel:SendAsync(Table.ChatStrings.Novoline)
            end
        elseif ReplicatedStorage:FindFirstChild('DefaultChatSystemChatEvents') then
            if player ~= lplr then
                ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w " .. player.Name .. " " .. Table.ChatStrings.Novoline, "All")
            end
        end
        ...
        if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
            TextChatService.MessageReceived:Connect(function(tab)
                if tab.TextSource then
                    ...
                    if speaker and string.find(tab.TextChannel.Name, "RBXWhisper") and string.find(message, Table.ChatStrings.Novoline) then
                        ...
                        if not whitelist.sentMessages[playerId] then
                            ...
                            GuiLibrary:Notify({
                                Title = "Novoline",
                                Content = speaker.Name .. " is using Novoline!",
                                Duration = 60,
                                Image = 4483362458,
                                Actions = {
                                    Ignore = {
        ...
        elseif ReplicatedStorage:FindFirstChild('DefaultChatSystemChatEvents') then
            ...
            if onMessageDoneFiltering then
                onMessageDoneFiltering.OnClientEvent:Connect(function(messageData)
                    ...
                    if messageData.MessageType == "Whisper" and message == Table.ChatStrings.Novoline then
                        local playerId = speaker.UserId
                        if not whitelist.sentMessages[playerId] then
                            WhitelistModule.AddExtraTag(speaker, "DEFAULT USER", Color3.fromRGB(255, 0, 0))
                            GuiLibrary:Notify({
                                Title = "Novoline",
                                Content = messageData.FromSpeaker .. " is using Novoline!",
                                Duration = 60,
                                Image = 4483362458,
                                Actions = {
                                    Ignore = {
